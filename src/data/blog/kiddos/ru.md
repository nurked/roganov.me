...
А вот как настоящая штука выглядит уже сегодня, прямо из сборки:

![Экран загрузки KidDOS 0.3.0: ЭЛТ-терминал, ребёнок печатает hi, машина спрашивает имя, а строка наставника пурпурным предлагает набрать help](/img/blog/kiddos-hi.webp)
...
У меня есть план. Rust, виртуальная файловая система, встроенный Бейсик, wasmtime для компилируемых языков, формат картриджей, чтобы игры были просто папками, которые кидаешь внутрь. Этапы расписаны.

И вот что я чуть не забыл сказать: это не просто план. Оно уже построено. Первые два этапа готовы, и код лежит в открытом доступе прямо сейчас: [github.com/nurked/kiddos](https://github.com/nurked/kiddos). Шелл, виртуальный диск, мануал, двенадцать уроков с наставником, который следит за тем, что ты печатаешь, `edit`, Бейсик и семь картриджей — пещерный квест, угадай число, змейка, виселица, тренажёр печати, тетрис и сокобан, всё на Бейсике, который ребёнок может прочитать и поменять. Установщиков пока нет, так что нужен Rust на машине, а дальше одна строчка:
...
С тех пор у проекта появился свой дом: [kiddos.dev](https://kiddos.dev/), с [подписанными сборками для Mac, Windows и Linux](https://kiddos.dev/download/), [страницей для родителей](https://kiddos.dev/parents/) о том, как выйти из полноэкранного режима, и [педагогической частью аргумента](https://kiddos.dev/how-to-teach-kids-programming/), написанной как следует (по-английски).

Но сразу хочу извиниться: большая часть этого репозитория написана с помощью ИИ. Это первый, очень сырой прототип, доказательство концепции, а не готовая вещь. Сюжетные линии, диалоги, мелкие детали, из-за которых кажется, что внутри машины кто-то есть, — всё это требует ОЧЕНЬ много ручной работы, и я это полностью признаю. Читайте его как набросок идеи, а не как идею, сделанную правильно.
...
Вы бы склонировали, запустили и рассказали мне, что из этого вышло?
