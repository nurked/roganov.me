...
> **Примечание из 2026 года:** статья написана в октябре 2021-го по исходникам runtime Go из ветки master того времени, которая стала Go 1.18. Устройство планировщика с тех пор не поменялось, но детали разъехались: номера строк ниже не совпадают ни с одним релизом, `netpollblock` и `netpollunblock` переписаны, а начиная с Go 1.25 `GOMAXPROCS` по умолчанию учитывает лимит CPU контейнера и пересчитывается на ходу. Читайте сниппеты как снимок 2021 года.
...
Ага, то есть мы не просто запускаем планировщик, мы стартуем нашу программу. К моменту запуска `schedinit` функция `osinit` уже отработала: именно она спрашивает у ОС, сколько у нас ядер (на Linux через `sched_getaffinity`), и записывает ответ в `ncpu`. Исходники этой функции разные для разных ОС, посему здесь мы в детали лезть не будем, ибо их очень уж много.

Дальше, уже внутри `schedinit`, делаем вот что:
...
Несмотря на название, `cpuinit` ничего не считает. Он читает `GODEBUG` и выясняет, какие возможности процессора нам разрешено использовать (SSE4.1, FMA, атомарные операции на ARM64), и делает это с помощью приличного количества ассемблера.
