...
> **Примечание из 2026 года:** статья написана в январе 2021-го по коммиту rustc `b12290861` (1.51.0-nightly), и все сниппеты ниже совпадают с теми исходниками. С тех пор `rustc_typeck` разделили на `rustc_hir_analysis` и `rustc_hir_typeck`, `rustc_mir` разбили на `rustc_borrowck`, `rustc_mir_transform` и компанию, между HIR и MIR появился THIR, оператор `?` теперь разворачивается через `Try::branch`, а `-Z ast-json` заменили на `-Z unpretty=ast-tree`. Относитесь к путям файлов как к снимку 2021 года.
...
Вот, собственно говоря, и всё, ребята! LLVM за пределами нашей видимости. Он вкомпилирован в сам rustc и перегоняет IR в объектные файлы. А на моей операционной системе линкер из Visual Studio Build Tools берёт на себя контроль и собирает из этих объектников обычный бинарник.
