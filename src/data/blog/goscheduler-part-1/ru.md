---
title: "Лезем в сорцы компилятора — как работает goscheduler (Часть I)"
slug: "goscheduler-part-1"
date: 2021-10-04
description: "Первая часть серии о планировщике задач в Go: что происходит с потоками ОС, почему 180000 потоков убивают систему и при чём тут захват работы."
lang: "ru"
...
> **Примечание из 2026 года:** статья написана в октябре 2021-го. Цитируемая ниже документация Майкрософт с тех пор переписана (цифра в 2048 потоков теперь явно относится к 32-битным системам), а цифры с моего ноутбука такие, какими были в тот день. Детали про runtime Go актуальны и для Go 1.25.
...
> — Rob Pike, «Concurrency is not parallelism», Heroku Waza, январь 2012
>
...
> Количество потоков, создаваемых процессом, ограничено количеством виртуальной памяти. По умолчанию каждый поток создаётся с мегабайтом стекового пространства. Поэтому вы можете создать не более 2048 потоков. Если вы уменьшите размер стэка, то сможете создать больше потоков. Хотя ваше приложение будет иметь лучшую производительность, если вы создадите один поток на каждый процессор и построите очереди задач, для которых приложение будет сохранять контекст.
...
Тестовый стенд: Intel Core i7 десятого поколения. Продвинутый мобильный процессор, более чем скромный по серверным меркам, но для тестов подойдёт.
...
Если вы хотите протестировать свою систему без компилирования программ на С++, то можете воспользоваться замечательной утилитой `testlimit64 -t`, написанной Марком Руссиновичем.
...
Практически любой фреймворк несёт в себе какое-то подобие Thread Pool в С# и Thread Pool в Java.

### Первые шаги

В голанге, вместо того, чтобы позволять вам крутить все эти потоки и пулы, разработчики пошли дальше и закрыли возможность напрямую создавать потоки в ОС. Вместо этого нам был предоставлен инструмент создания горутин и планировщик задач, разработанный специально для го.
...
Ладно, так и быть, я признаюсь, нет в мире ни одной статьи о планировщике в голанге, где не была бы показана картинка с треугольничками, квадратиками и кружочками. Куда ни ткни, там эти треугольнички, квадратики и кружочки.

Мы посмотрим на все эти G, P и M, но попозже.

Но нам этого мало. Как на самом деле работает планировщик? Что же, ответить на этот вопрос не так-то просто. Давайте залезем в репозиторий go/golang и найдём там настоящий планировщик. `src/runtime/proc.go`. Вот где суслик зарыт.
...
А если вы посмотрите на то, как устроен кеш любого процессора, то поймёте, что проблема может быть более глубокой. В нашем ненатуральном тесте, чуть выше, мы не пользовались памятью в процессах, не сохраняли никаких данных и не производили никаких вычислений. Но, любая настоящая программа будет записывать и читать данные из оперативной памяти. Данные, прочитанные из памяти, при исполнении программы будут помещаться в кеш процессора. Поэтому, когда операционная система решает, что определённый поток наигрался на процессоре, и ему пора пойти отдохнуть, работы предстоит немало. Вам нужно не только изменить инструкции, выполняемые процессором, но и переписать кеши.

А кеш у нас не один. У каждого ядра три своих: L1i и L1d, кеши инструкций и данных первого уровня, плюс кеш второго уровня. А кеш третьего уровня все ядра делят между собой.
...
Для того чтобы выяснить, когда надо производить операцию переключения контекста, ОС использует планировщик задач.

Исходя из этих данных, мы знаем, что Windows использует упреждающий планировщик задач, основанный на приоритетах. В зависимости от текущей конфигурации системы выделяются определённые кванты времени. Промежутки времени, в которые происходит выполнение потока. Каждый поток выполняется только в течение этого кванта времени, после этого снимается с процессора и заменяется другим потоком, в зависимости от приоритета.

Кстати, в зависимости от этого же приоритета, поток может даже и не закончить свой квант, если появится поток с большим приоритетом.
